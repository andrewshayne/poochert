...
export default function App() {
  ...
  const handleAdminLogin = async () => {
    ...
    try {
      const res = await fetch(`/${slug}/api/verify-admin`, {
        method: 'POST',
  ...
  const handleSavePhotoChanges = async (e) => {
    ...
    try {
      ...
      if (res.ok) { setSelectedPhoto(null); fetchPhotos(); } else { alert('Failed to update photo.'); }
  ...
  const handleDeletePhoto = async (e, photoId) => {
    ...
    try {
      ...
      if (res.ok) { fetchPhotos(); } else { alert('Failed to delete photo.'); }
